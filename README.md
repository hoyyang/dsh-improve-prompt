...
| **悬停** | 上浮 1.5px、放大 1.04、描边提亮（慢速 3.4s 一圈）、光池增亮、外沿窄光晕变强；**无**扫动条、**无**呼吸、**无**彗星尾 |
| **点按** | 回落到 `scale(.94)`、亮度回收、描边光加速 |
| **进行中** | **与悬停明显不同**：描边变成带长尾的**彗星**绕行、底边有**扫动的能量条**（不确定进度）、整块芯片**呼吸充能**（scale 1→1.028）、图标自转、光池呼吸 |
...
最差 **9.83:1**（WCAG AA 正文要求 ≥4.5:1）。禁用态同样受测：WCAG 豁免非活性控件，本插件的约束不豁免。
