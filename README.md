...
**设计原则：先是精确的形状，然后才是光。** 28px 的控件活在密集工具栏里，靠大面积发光只会糊成雾。这个按钮的前两版都栽在这上面——一版只有一根发丝描边、没有材质；一版用了一大片光晕，把整行输入区糊住。所以现在的规则是：**药丸是一个精确的物件，所有光源都收在它内部。**

| 层 | 做法 |
|---|---|
| **底板** | 闲置是幽灵态（与相邻控件一致）；悬停起切到主题实色 + 顶部 1px 高光 + 清晰 1px 描边 + 10px 以内的彩色投影（有悬浮感，但不外溢） |
| **描边光** | 1px 圆锥渐变沿边框带走一圈，只带 2px `drop-shadow` —— 读作"边上跑过的光"，不是轮廓线也不是雾 |
| **图标** | 内联 SVG，**固定青色**（装饰件不跟随文字色，否则浅色主题下会变近黑、与身后的火花打架）+ 3px 紧致辉光；悬停放大 1.14 并转 90°，进行中自转 |
| **火花** | 生成素材（四角星芒）18px 贴在图标位，作为图标的气场；**盒宽 3+18=21px，标签盒从 30px 起**，留 9px 净空 |
| **文字** | 最高层；主题色；**不加 text-shadow**（实测底板对比度 13.74:1 不需要它，而深字加深影在浅色主题下会糊成灰晕） |

| 状态 | 表现 |
|---|---|
| **闲置** | 幽灵药丸 + 一枚柔和青色星芒 |
| **悬停** | 实色底板 + 描边光开始绕行 + 图标放大转 90° + 投影上浮 1px |
| **点按** | 落回 `scale(.96)`、内阴影加深、描边光加速 |
| **进行中** | 描边光加速成跑环 + 图标自转 + 火花脉动 |
| **禁用 / 焦点** | 火花与描边熄灭、图标转暗；`focus-visible` 保留键盘焦点环 |
...
1. **没有任何光源在文字底下**：生成素材只贴在图标位（净空 9px）；描边光用 `mask-composite` 只占 1px 边框带，永远进不了内边距区；药丸之外不放任何光层。
2. **亮态即实色底板**：只要描边光亮起（悬停 / 点按 / 进行中），底板必为主题实色。
3. **实测**（`npm run harness:button` 生成自检页，页面内置断言）：

| 状态 | 对比度 | 底板 | 火花压字 | 文字在内边距内 |
|---|---|---|---|---|
| dark 闲置 | 8.67:1 | 透明 | 否 | 是 |
| dark 悬停 / 点按 | 13.74:1 | **实色** | 否 | 是 |
| dark 进行中 | 13.74:1 | **实色** | 否 | 是 |
| dark 禁用 | 6.15:1 | 透明 | 否 | 是 |
| light 闲置 | 7.86:1 | 透明 | 否 | 是 |
| light 悬停 / 点按 | 13.74:1 | **实色** | 否 | 是 |
| light 进行中 | 16.74:1 | **实色** | 否 | 是 |
| light 禁用 | **4.90:1** | 透明 | 否 | 是 |

最差 **4.90:1**（WCAG AA 正文要求 ≥4.5:1），全部状态通过。禁用态也照测——WCAG 豁免非活性控件，但本插件的约束不豁免，所以禁用色从 `label-dimmed` 换成 `label-secondary` + `opacity:.82`（原为 2.63:1，不达标）。

自检脚本计算**有效前景**（把祖先链上的组不透明度折算进去，否则禁用态的 `.82` 会被忽略、报出偏高的数字），按 WCAG 相对亮度算比值，并逐状态断言"火花盒与文字盒不相交""文字盒在药丸内边距内""亮态底板为实色"。
