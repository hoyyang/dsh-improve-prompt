...
**精确的形状优先，光次之，且所有光源都收在芯片内部**（外壳 `overflow:hidden`）。底板是**恒深海军蓝玻璃**，两个主题同一块板、不反白；描边是宽度无关的 `conic-gradient`；配色是插件自己的青 → 电光蓝 → 金（与 banner 同源）。主按钮与档位切换 **▾** 融合为一颗**分体玻璃芯片**：一块底板两个区，档位区自带电光蓝图池，中间 1px 发丝分隔线只分开命中区、绝不穿过字形。

| 状态 | 表现 |
|---|---|
| 闲置 | 深色玻璃芯片（主区青色光池 + 档位区电光蓝图池）+ 完整渐变描边 + 发丝分隔线 |
| 悬停 | 上浮 1.5px、放大 1.04、描边提亮并慢速绕行（3.4s/圈）；悬停档位区时该区浮起玻璃高光 |
| 点按 | 回落到 `scale(.94)`、亮度回收、描边加速 |
