...
**精确的形状优先，光次之，且所有光源都收在药丸内部**（外壳 `overflow:hidden`）。底板是**恒深海军蓝玻璃**，两个主题同一块板、不反白；描边是宽度无关的 `conic-gradient`；配色是插件自己的青 → 电光蓝 → 金（与 banner 同源）。

| 状态 | 表现 |
|---|---|
| 闲置 | 深色玻璃芯片 + 完整渐变描边 + 图标处一枚青色光池 |
| 悬停 | 上浮 1.5px、放大 1.04、描边提亮并慢速绕行（3.4s/圈） |
...
没有任何光源在文字底下——火花只占图标位（净空 9px）、描边被 `mask-composite` 限制在 1px 边框带、能量条只占底部 2px（y25..27，文字盒 y8..20）。底板永不半透明，所以对比度在两主题、全状态下都成立。对比度由 **3x 截图的像素**算出（底板是渐变，计算色值回答不了）：
...
最差 **8.77:1**（WCAG AA 正文要求 ≥4.5:1），禁用态同样受测。`npm run harness:button` 会生成一个把本插件按钮与 dsh-plan-board 真实按钮并排的对照页，可复现全部结论。
