 ...
 */
export function extractAnchors(draft, limit = MAX_ANCHORS) {
    ...
    for (const { kind, pattern, claimOnly } of MATCHERS) {
        ...
        while ((match = re.exec(text)) !== null) {
            ...
            if (value.length < MIN_LENGTH || value.length > MAX_CODE_LENGTH)
                continue;
 ...
 */
export function anchorSurvives(anchor, normalizedEnhanced, rawEnhanced, strippedEnhanced) {
    if (rawEnhanced.includes(anchor.text))
        return true;
    if (normalizedEnhanced.includes(normalizeForMatch(anchor.text)))
        return true;
