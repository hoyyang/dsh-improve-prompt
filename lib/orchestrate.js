...
export async function improveDraft(deps, cfg, input) {
    ...
    if (input.draft.trimStart().startsWith('/'))
        return { ok: false, code: ERR.COMMAND, message: '斜杠命令不参与改写' };
    ...
    const verdict = cfg.lengthGate ? lengthVerdict(input.draft, enhanced, maxRatio) : { kind: 'ok', ratio: 1 };
