...
const boxes = [
  ...
  '    const label = document.querySelector("#mine-" + theme + " .dip-label")',
  '    const pill = document.querySelector("#mine-" + theme + " .dip-btn")',
  '    if (!label || !pill) continue',
  '    const lb = label.getBoundingClientRect(); const pb = pill.getBoundingClientRect()',
  '    out[theme] = { label: { x: lb.x, y: lb.y, w: lb.width, h: lb.height }, pill: { x: pb.x, y: pb.y, w: pb.width, h: pb.height } }',
  '  }',
  '  return out',
  '}',
  '<\/script>',
