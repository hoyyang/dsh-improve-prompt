...
export function normalizeOutput(raw: string): string {
  ...
  if (fence !== null && fence[1] !== undefined && fence[1].trim() !== '') text = fence[1].trim()
  ...
  if (quote !== null && quote[2] !== undefined && !quote[2].includes(quote[1]!)) text = quote[2].trim()
  ...
  while (lines.length > 1) {
    ...
    if (last === '' || DIVIDER.test(last)) { lines.pop(); continue }
    break
  }
  text = lines.join('\n').trim()

  // A rewrite that is only a divider or a stray fence is not usable.
  if (text === '' || DIVIDER.test(text) || /^```/.test(text) && text.length < 4) return ''
  return text
}
